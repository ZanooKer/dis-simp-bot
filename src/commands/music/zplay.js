...
module.exports = {
  ...
  async execute(bot, interaction) {
    ...
    if (!channel.viewable)
      return bot.say.wrongEmbed(
        interaction,
        "I need `View Channel` permission."
      );

    if (!channel.joinable)
      return bot.say.wrongEmbed(
        interaction,
        "I need `Connect Channel` permission."
      );

    if (channel.full)
      return bot.say.wrongEmbed(
        interaction,
        "Can't join, the voice channel is full."
      );
    ...
    if (interaction.guild.members.me?.voice?.mute)
      return bot.say.wrongEmbed(
        interaction,
        "Please unmute me before playing."
      );
