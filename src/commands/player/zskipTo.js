...
module.exports = {
  ...
  async execute({ interaction }) {
    ...
    if (!track && !number)
      return replyText(
        interaction,
        `You have to use one of the options to jump to a song`
      );

    if (track) {
      const skipToTrack = queue.tracks
        .toArray()
      ...
      return replyEmbedText(interaction, {
        text: `Skip to ${skipToTrack.title} | ${skipToTrack.author}  ✅`,
      });
    }
    if (number) {
      ...
      if (!trackName)
        return replyText(interaction, `This track is not existed`);
      queue.node.skipTo(index);
      return replyEmbedText(interaction, {
        text: `Skip to ${trackName} | ${author} ✅`,
      });
    }
  },
};
